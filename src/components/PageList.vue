	...
	<div class="page-list">
		...
		<ul class="scrollbar">
			<li v-for="(item,index) in page_list" :key="index" @click="changePage(index, item.path, item.type, item.id)" :class="{'li-active': index==page_list_index}">
				<span>{{item.name}}</span>
				<div class="img">
					<img v-if="item.type != 'index' && item.type != 'article' && item.type != 'us'" class="icon-img" @click.stop="deletePage(item.id)"
					 src="../assets/icom-img/delete-1.png" alt="">
				</div>
			</li>
		</ul>
		...
		methods: {
			...
			deletePage(id) {
				...
				this.$confirm('删除不可恢复，是否继续', '提示', {
					...
					sortableModel.deletePage(data).then(res => {
						if (res.msg == "成功删除") {
							this.$message({
								message:res.msg,
								type: 'success'
